...
const TEXT_QUERY = gql`
  query TextsQuery($textId: Int) {
    ...
    activeTagsForNote(noteId: $textId) {
      nodes {
        name
        slug
      }
    }
    instructionsForNote(noteId: $textId) {
      nodes
    }
  }
`;
...
export default ({ match }) => {
  const { loading, error, data } = useQuery(TEXT_QUERY, {
    variables: { textId: parseInt(match.params.id, 10) },
  });
  ...
  return (
    <article>
      ...
      <section>
        <Body
          cachedBodyHtml={data.text.cachedBodyHtml}
          annotationsCount={data.text.annotationsCount}
        />
        <aside>
          {data.text.role !== "unregistered" && (
            <p>
              <em>This note is only available to {data.text.role} users.</em>
            </p>
          )}
          <section id="tags">
            <ul>
